...
def get_validation_prompt(args, image, model, device='cuda'):
    ...
    vocab_words = load_vocab("E:/Sydney_study/5329/A2/flickr/vocab_words.txt")
    generator = PromptGenerator(output_dim=len(vocab_words)).to(device)
    generator.load_state_dict(torch.load("E:/Sydney_study/5329/A2/flickr/model/generator.pth", map_location=device))
    generator.eval()
    ...
    def bind_adjectives_to_nouns(objects, adjectives, clip_model, device):
        results = []
        for noun in objects:
            best_adjs = []
            best_score = -1
            for i in range(1, min(4, len(adjectives)+1)):
                for combo in itertools.combinations(adjectives, i):
                    ...
                    with torch.no_grad():
                        ...
                        score = text_feat.float().norm(p=2, dim=-1).mean().item()
                    if score > best_score:
                        best_score = score
                        best_adjs = combo
    ...
    prompt_fragments = bind_adjectives_to_nouns(nouns, adjectives, clip_model, device)
