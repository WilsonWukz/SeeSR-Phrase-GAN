...
from Gans_predict import predict_adjectives, load_vocab
...
def get_validation_prompt(args, image, model, device='cuda'):
    ...
    # 加载 Prompt-GAN 模型 & vocab
    vocab_words = load_vocab("E:/Sydney_study/5329/A2/COCO/vocab_words.txt")
    generator = PromptGenerator(output_dim=len(vocab_words)).to(device)
    generator.load_state_dict(torch.load("E:/Sydney_study/5329/A2/flickr/model/coco_generator2.pth", map_location=device))
    generator.eval()
    ...
    nouns = [o.strip() for o in res[0].split(',') if o.strip()]

    # 融合形容词与名词（使用 CLIP 文本编码相似度）
    def bind_adjectives_to_nouns(objects, adjectives, clip_model, img_feat, device):
        results = []
        used_adjs = set()
        for noun in objects:
            best_adjs = []
            best_score = -1
            for i in range(1, min(4, len(adjectives) + 1)):
                for combo in itertools.combinations(adjectives, i):
                    if any(adj in used_adjs for adj in combo):
                        continue  # 避免重复形容词被多个 noun 用到
                    prompt = " ".join(combo + (noun,))
                    text_inputs = clip.tokenize([prompt]).to(device)
                    with torch.no_grad():
                        text_feat = clip_model.encode_text(text_inputs)
                        sim = F.cosine_similarity(img_feat, text_feat, dim=-1).mean().item()
                    if sim > best_score:
                        best_score = sim
                        best_adjs = combo
            used_adjs.update(best_adjs)
            results.append(f"{' '.join(best_adjs)} {noun}")
        return results
    ...
    # 使用图像特征与文本相似度来绑定形容词与名词
    prompt_fragments = bind_adjectives_to_nouns(nouns, adjectives, clip_model, img_feat, device)
    final_prompt = ", ".join(prompt_fragments)
    validation_prompt = f"{final_prompt}, {args.prompt},"
    # print("融合后的硬提示:", validation_prompt)
        ...
        for image_idx, image_name in enumerate(image_names[:]):
            ...
            for sample_idx in range(args.sample_times):
                with torch.autocast("cuda"):
                    image = pipeline(
                        validation_prompt, validation_image, num_inference_steps=args.num_inference_steps,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--seesr_model_path", type=str, default="preset/models/seesr")
    ...
    parser.add_argument("--image_path", type=str, default="preset/datasets/test_datasets")#
    parser.add_argument("--output_dir", type=str, default="preset/datasets/output")
    ...
    parser.add_argument("--start_point", type=str, choices=['lr', 'noise'],
                        default='lr')  # LR Embedding Strategy, choose 'lr latent + 999 steps noise' as diffusion start point.
